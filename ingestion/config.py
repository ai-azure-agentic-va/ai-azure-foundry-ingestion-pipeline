...
class Settings:
    ...
    def __init__(self):
        ...
        # Embeddings
        self.FOUNDRY_EMBEDDING_DEPLOYMENT: str = os.environ.get("FOUNDRY_EMBEDDING_DEPLOYMENT", "text-embedding-3-large")
        self.FOUNDRY_EMBEDDING_DIMENSIONS: int = int(os.environ.get("FOUNDRY_EMBEDDING_DIMENSIONS", "3072"))
        self.FOUNDRY_EMBEDDING_MODEL: str = os.environ.get("FOUNDRY_EMBEDDING_MODEL", "text-embedding-3-large")
        ...
        self.EMBEDDING_BACKOFF_CEILING: float = float(os.environ.get("EMBEDDING_BACKOFF_CEILING", "60.0"))
        self.EMBEDDING_TPM_RESERVE_FRACTION: float = float(os.environ.get("EMBEDDING_TPM_RESERVE_FRACTION", "0.2"))
        self.EMBEDDING_MAX_BATCH_TOKENS: int = int(os.environ.get("EMBEDDING_MAX_BATCH_TOKENS", "8000"))
        self.EMBEDDING_ESTIMATED_INSTANCES: int = int(os.environ.get("EMBEDDING_ESTIMATED_INSTANCES", "30"))

        # PII Detection (Azure AI Language)
        self.FOUNDRY_PII_ENDPOINT: str | None = os.environ.get("FOUNDRY_PII_ENDPOINT")
        self.PII_CONFIDENCE_THRESHOLD: float = float(os.environ.get("PII_CONFIDENCE_THRESHOLD", "0.8"))
        ...
        # "halt" = skip document on PII failure, "proceed" = index unredacted
        ...
        # File size guard — reject before loading into memory
        self.MAX_FILE_SIZE_MB: int = int(os.environ.get("MAX_FILE_SIZE_MB", "500"))

        # Document Intelligence (fallback for image OCR)
