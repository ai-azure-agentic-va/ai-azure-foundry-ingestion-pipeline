...
def _build_index_schema(index_name: str) -> SearchIndex:
    ...
    fields = [
        ...
        SimpleField(
            name="total_chunks", type=SearchFieldDataType.Int32, filterable=True
        ),
        SimpleField(
            name="page_number",
            type=SearchFieldDataType.Int32,
...
class SearchPusher:
    ...
    def __init__(
        ...
        logger.info(
            f"[SearchPusher] Initialized: endpoint={self.endpoint}, index={self.index_name}"
        )
    ...
    def push(self, chunks: list[dict], batch_size: int = 100) -> dict:
        ...
        logger.info(
            f"[SearchPusher] Pushing {len(chunks)} chunks to index '{self.index_name}'"
        )
        ...
        logger.info(
            f"[SearchPusher] Push complete: {total_success} succeeded, {total_failed} failed"
        )
