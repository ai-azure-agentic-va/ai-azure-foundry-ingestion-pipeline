"""Azure Functions entry point for custom-processing.

Custom Libraries pipeline: PyMuPDF, python-docx, Presidio, etc.
Active when DOC_PROCESSING=CUSTOM_LIBRARIES (or this Function App is deployed).
...
def _get_pipeline():
    """Get the CustomDocPipeline instance (thread-safe)."""
    global _pipeline
    if _pipeline is None:
        with _pipeline_lock:
            if _pipeline is None:
                from modules.pipeline import CustomDocPipeline
                _pipeline = CustomDocPipeline()
...
def process_new_document(event: func.EventGridEvent):
    ...
    Uses CustomDocPipeline (PyMuPDF, Presidio, etc.)."""
...
def health_check(req: func.HttpRequest) -> func.HttpResponse:
    ...
    return func.HttpResponse(
        json.dumps({
            "status": "healthy",
            "service": os.environ.get("FUNCTION_APP_NAME", "custom-processing"),
            "processing_path": "CUSTOM_LIBRARIES",
